...
## 节点引用
@onready var display_icon: TextureRect = %DisplayIcon
@onready var item_count_label: Label = %ItemCountLabel

## 物品资源 
@export var item_slot: ItemSlot
@export_file_path(".png") var default_item_icon: String
...
func update_item_display() -> void:
	if item_slot:
		if item_slot.item.item_icon:
			display_icon.texture = item_slot.item.item_icon
		else:
			display_icon.texture = load(default_item_icon)
...
func _update_item_count_label() -> void:
	if not item_slot or item_slot.item_count < 1:
